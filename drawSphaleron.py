...
for line in txt:
    ...
    if (line[0]=="SPHA"):
        ...
        if(E_spha==10.0):
            PEF =  limit/0.268
        print "%s   xsec_limit = %.3f  PEF_limit=%.3f"%(E_spha,limit, PEF)
...
g_spha.GetYaxis().SetTitleSize(0.03)
g_spha.GetXaxis().SetTitleOffset(1.2)
g_spha.GetYaxis().SetTitleOffset(1.25)
g_spha.GetXaxis().SetLabelSize(0.05)
g_spha.GetYaxis().SetLabelSize(0.03)
leg.Draw("same")
